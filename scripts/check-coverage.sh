...
binary="$(find .build -type f -path '*CakePackageTests.xctest/Contents/MacOS/CakePackageTests' | head -n 1)"
...
echo "[coverage] Effective coverage (Sources/CakeKit non-UI scope): ${coverage_pct}% (${covered_lines}/${total_lines} lines)"
