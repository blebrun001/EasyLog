...
func highConcentrationProducesMorePointSymbolsAndStaysInsideUnit() {
    ...
    let scene = EasyLogRenderer().makeScene(project: Project(units: [unit]))
...
func syntheticComparisonAlignsUnitsInSharedAltitudeReference() {
    ...
    let scene = SyntheticComparisonSceneBuilder.make(logs: [logA, logB], selectedLogIndex: 0, renderer: EasyLogRenderer())
