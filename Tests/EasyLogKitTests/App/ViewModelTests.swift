...
func openProjectViaDialogLoadsProjectAndUpdatesState() throws {
    ...
    let tempURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appending(path: "easylog-open-\(UUID().uuidString).json")
...
func saveProjectWritesToStoreAndTracksURL() {
    ...
    let saveURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appending(path: "easylog-save-\(UUID().uuidString).json")
...
func exportProjectPassesRequestedFormatAndDPI() {
    ...
    let exportURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appending(path: "easylog-export-\(UUID().uuidString).jpg")
...
func exportAllProjectsEmitsOneFilePerLogAndResolvesFilenameCollisions() {
    ...
    let folder = URL(fileURLWithPath: NSTemporaryDirectory())
        .appending(path: "easylog-export-all-\(UUID().uuidString)")
...
func defaultZoomModeIsFitWindow() {
    let suiteName = "easylog-tests-\(UUID().uuidString)"
...
func startupIgnoresStalePersistedManualZoomAndUsesFitWindow() {
    ...
    defaults.set(1.0, forKey: "easylog.viewer.zoom")
    defaults.set("manual", forKey: "easylog.viewer.zoomMode")
    defaults.set(false, forKey: "easylog.viewer.autoAdjust")
...
func colorProfilesBootstrapWithSingleDefaultProfile() {
    let suiteName = "easylog-vm-preset-tests-\(UUID().uuidString)"
    let defaults = UserDefaults(suiteName: suiteName)!
    defer {
        defaults.removePersistentDomain(forName: suiteName)
    }
